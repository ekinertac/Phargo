...
// ---- subsystem modules (carved out of the original monolithic lib.rs) ------
// Each is a self-contained, mostly-pure block that the Engine calls into:
mod bcmath; // arbitrary-precision decimal arithmetic (bc* functions)
mod datetime; // civil-calendar date/time math (date/mktime/strtotime)
mod dump; // var_dump / print_r / var_export formatters
mod hash; // md5 / sha1 / crc32 / base64
mod json; // json_encode / json_decode
mod regex; // from-scratch backtracking regex VM (preg_*)
...
pub(crate) type ObjRef = Rc<RefCell<Obj>>;
