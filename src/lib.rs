...
class SimpleXMLElement implements Iterator, ArrayAccess, Countable {
    public $__name = ""; public $__text = ""; public $__attrs = []; public $__children = [];
    private $__p = 0;
    public function __construct($xml = "") {
        if ($xml === "") { return; }
        $o = simplexml_load_string($xml);
        if ($o) {
            $this->__name = $o->__name; $this->__text = $o->__text;
            $this->__attrs = $o->__attrs; $this->__children = $o->__children;
        }
    }
    public function getName() { return $this->__name; }
    public function __toString() { return $this->__text; }
    public function __get($name) {
        foreach ($this->__children as $c) { if ($c->__name === $name) { return $c; } }
        return null;
    }
    public function children($ns = null) { return $this->__children; }
    public function attributes($ns = null) { return $this->__attrs; }
    public function offsetExists($k): bool { return isset($this->__attrs[$k]); }
    public function offsetGet($k): mixed { return $this->__attrs[$k] ?? null; }
    public function offsetSet($k, $v): void { $this->__attrs[$k] = $v; }
    public function offsetUnset($k): void { unset($this->__attrs[$k]); }
    public function count(): int { return count($this->__children); }
    public function rewind(): void { $this->__p = 0; }
    public function valid(): bool { return $this->__p < count($this->__children); }
    public function current(): mixed { return $this->__children[$this->__p]; }
    public function key(): mixed { return $this->__children[$this->__p]->__name; }
    public function next(): void { $this->__p = $this->__p + 1; }
    public function asXML() { return __phargo_xml_dump($this); }
    public function saveXML() { return __phargo_xml_dump($this); }
}
...
class DateInterval {
    ...
    public $days = false; public $invert = 0;
...
impl Engine {
    ...
    fn parse_assignment(&mut self) -> R<Value> {
        ...
        if self.peek() == Some('$') {
            ...
            if let Some(aop) = self.peek_assign_op() {
                ...
                let rhs = self.parse_assignment()?;
                if indices.is_empty() {
                    // `.=` on a string var: append in place (avoids O(n^2) from
                    // cloning the whole left side each time, e.g. in a loop).
                    if aop == ".=" && self.live {
                        if let Some(Value::Str(s)) = self.vars.get_mut(&name) {
                            let r = rhs.to_php_string();
                            if s.len() + r.len() > 64 * 1024 * 1024 {
                                return Err(EngineError("string concatenation too large".into()));
                            }
                            s.push_str(&r);
                            // The value of a `.=` is the new string, but it's almost
                            // always used as a statement (discarded); returning the
                            // clone would re-introduce O(n^2). Return it only when
                            // small enough that the clone is cheap.
                            if s.len() <= 4096 {
        ...
        match self.peek() {
            ...
            Some('(') => {
                if let Some(ct) = self.try_cast() {
                    let v = self.parse_unary()?;
                    // (string) on an object must invoke __toString
                    if matches!(ct.as_str(), "string" | "binary")
                        && matches!(v, Value::Object(_))
                    {
                        return Ok(Value::Str(self.stringify(&v)?));
                    }
    ...
    fn apply_binary(&self, op: &str, l: Value, r: Value) -> R<Value> {
        ...
        let v = match op {
            "." => {
                let (ls, rs) = (l.to_php_string(), r.to_php_string());
                // Guard runaway `.=`-in-a-loop from allocating gigabytes.
                if ls.len() + rs.len() > 64 * 1024 * 1024 {
                    return Err(EngineError("string concatenation too large".into()));
                }
                Str(format!("{ls}{rs}"))
            }
    ...
    fn call_function(&mut self, name: &str, args: Vec<Value>) -> R<Value> {
        ...
        let v = match name.to_ascii_lowercase().as_str() {
            ...
            // ---- SimpleXML ---------------------------------------------------
            "simplexml_load_string" => match xml_parse(&arg(0).to_php_string()) {
                Some(node) => xml_to_simplexml(&node),
                None => Value::Bool(false),
            },
            "simplexml_load_file" => match std::fs::read_to_string(arg(0).to_php_string()) {
                Ok(s) => match xml_parse(&s) {
                    Some(node) => xml_to_simplexml(&node),
                    None => Value::Bool(false),
                },
                Err(_) => Value::Bool(false),
            ...
            "__phargo_xml_dump" => {
                Value::Str(format!("<?xml version=\"1.0\"?>\n{}\n", xml_dump_value(&arg(0))))
            }
            // ---- bcmath (arbitrary-precision decimal) ------------------------
            "bcscale" => {
                if matches!(arg(0), Value::Null) {
                    Value::Int(self.bc_scale as i64)
                } else {
                    self.bc_scale = (to_long(&arg(0)).max(0) as usize).min(BC_SCALE_MAX);
                    Value::Bool(true)
                }
            }
            "bcadd" | "bcsub" | "bcmul" | "bccomp" | "bcdiv" | "bcmod" | "bcpow" | "bcsqrt" => {
                let a = arg(0).to_php_string();
                let lname = name.to_ascii_lowercase();
                // last arg is scale for most; bcsqrt(num, scale); bccomp(a,b,scale)
                let scale = if lname == "bcsqrt" {
                    args.get(1).map(to_long)
                } else {
                    args.get(2).map(to_long)
                }
                .map(|s| s.max(0) as usize)
                .unwrap_or(self.bc_scale)
                .min(BC_SCALE_MAX);
                let b = arg(1).to_php_string();
                match lname.as_str() {
                    "bcadd" => Value::Str(bc_add(&a, &b, scale)),
                    "bcsub" => Value::Str(bc_sub(&a, &b, scale)),
                    "bcmul" => Value::Str(bc_mul(&a, &b, scale)),
                    "bccomp" => Value::Int(bc_comp(&a, &b, scale)),
                    "bcdiv" => match bc_div(&a, &b, scale) {
                        Some(s) => Value::Str(s),
                        None => return Err(EngineError("bcdiv(): Division by zero".into())),
                    },
                    "bcmod" => match bc_mod(&a, &b, scale) {
                        Some(s) => Value::Str(s),
                        None => return Err(EngineError("bcmod(): Modulo by zero".into())),
                    },
                    "bcpow" => Value::Str(bc_pow(&a, &b, scale)),
                    "bcsqrt" => match bc_sqrt(&a, scale) {
                        Some(s) => Value::Str(s),
                        None => return Err(EngineError("bcsqrt(): negative number".into())),
                    },
                    _ => unreachable!(),
                }
            }
            ...
            "mktime" | "gmmktime" => {
                ...
                let h = if args.is_empty() { secs / 3600 } else { to_long(&arg(0)) };
                let mi = if args.len() < 2 { (secs % 3600) / 60 } else { to_long(&arg(1)) };
                let s = if args.len() < 3 { secs % 60 } else { to_long(&arg(2)) };
                let mon = if args.len() < 4 { cm } else { to_long(&arg(3)) };
                let day = if args.len() < 5 { cd } else { to_long(&arg(4)) };
                let year = if args.len() < 6 { cy } else { to_long(&arg(5)) };
                Value::Int(make_ts(h, mi, s, mon, day, year))
            }
            "checkdate" => {
                let (m, d, y) = (to_long(&arg(0)), to_long(&arg(1)), to_long(&arg(2)));
                Value::Bool(
                    (1..=12).contains(&m)
                        && (1..=32767).contains(&y)
                        && d >= 1
            ...
            "strtotime" => {
                let base = if args.len() > 1 { to_long(&arg(1)) } else { now_unix() };
                match php_strtotime(&arg(0).to_php_string(), base) {
                    Some(ts) => Value::Int(ts),
                    None => Value::Bool(false),
                }
            }
            "phargo_civil_add" => {
                let ts = to_long(&arg(0));
                let days0 = ts.div_euclid(86400);
                let secs0 = ts.rem_euclid(86400);
                let (y, mo, d) = civil_from_days(days0);
                let (dy, dm, dd) = (to_long(&arg(1)), to_long(&arg(2)), to_long(&arg(3)));
                let (dh, di, ds) = (to_long(&arg(4)), to_long(&arg(5)), to_long(&arg(6)));
                // years/months first, clamping the day to the target month length
                let total_months = (y * 12 + (mo - 1)) + dy * 12 + dm;
                let ny = total_months.div_euclid(12);
                let nmo = total_months.rem_euclid(12) + 1;
                let nday = d.min(days_in_month(ny, nmo));
                let base = days_from_civil(ny, nmo, nday) * 86400 + secs0;
                Value::Int(base + dd * 86400 + dh * 3600 + di * 60 + ds)
            }
            "phargo_date_diff" => {
                let (t1, t2) = (to_long(&arg(0)), to_long(&arg(1)));
                let invert = t1 > t2;
                ...
                for (k, v) in [("y", y), ("m", mo), ("d", d), ("h", h), ("i", mi), ("s", s), ("days", total_days)] {
                    r.set(AKey::Str(k.into()), Value::Int(v));
                }
                r.set(AKey::Str("invert".into()), Value::Int(invert as i64));
                Value::Array(r)
            }
            "__phargo_modify" => {
                let ts = to_long(&arg(0));
                let s = arg(1).to_php_string().to_ascii_lowercase();
            ...
            "microtime" => {
                if to_bool(&arg(0)) {
                    Value::Float(now_unix() as f64)
                } else {
                    Value::Str(format!("0.00000000 {}", now_unix()))
                }
            }
    ...
    fn double_quoted(&mut self) -> R<String> {
        self.pos += 1;
        // Collect the raw body (only resolving `\"` and the closing quote), then
        // run the shared interpolator (handles $var / $o->p / $a[k] / {$expr} and
    ...
    /// Evaluate a PHP expression given as a string (used by `{$...}` string
    /// interpolation). Appends to `src`, parses one expression, restores state.
    fn eval_subexpr(&mut self, code: &str) -> R<Value> {
        let start = self.src.len();
        self.src.extend(code.chars());
        let saved_pos = self.pos;
        self.pos = start;
    ...
    fn interpolate(&mut self, s: &str) -> R<String> {
        ...
        while i < chars.len() {
            ...
            // Complex syntax: `{$expr}` — evaluate the whole inner expression.
            ...
            if c == '$' && matches!(chars.get(i + 1), Some(d) if d.is_ascii_alphabetic() || *d == '_') {
                ...
                // simple syntax allows ONE level of `->prop` or `[key]`
                if chars.get(j) == Some(&'-') && chars.get(j + 1) == Some(&'>')
                    && matches!(chars.get(j + 2), Some(d) if d.is_ascii_alphabetic() || *d == '_')
                {
                    let ps = j + 2;
                    let mut pe = ps;
                    while matches!(chars.get(pe), Some(d) if d.is_ascii_alphanumeric() || *d == '_') {
                        pe += 1;
                    }
                    ...
                    let ks = j + 1;
                    let mut ke = ks;
                    while ke < chars.len() && chars[ke] != ']' {
                        ke += 1;
                    }
                    let raw: String = chars[ks..ke].iter().collect();
                    let key = if let Some(vn) = raw.strip_prefix('$') {
                        key_from_value(&self.vars.get(vn).cloned().unwrap_or(Value::Null))
                    } else if let Ok(n) = raw.parse::<i64>() {
                        AKey::Int(n)
                    } else {
                        AKey::Str(raw.trim_matches(['\'', '"']).to_string())
                    };
                    v = match &v {
...
fn php_date(fmt: &str, ts: i64) -> String {
    let days = ts.div_euclid(86400);
...
fn xml_parse(s: &str) -> Option<XmlNode> {
...
fn xml_dump_value(v: &Value) -> String {
    let o = match v {
        Value::Object(o) => o,
        _ => return String::new(),
    };
...
fn xml_to_simplexml(node: &XmlNode) -> Value {
...
const BC_SCALE_MAX: usize = 32768;

fn mag_trim(mut a: Vec<u8>) -> Vec<u8> {
    while a.len() > 1 && *a.last().unwrap() == 0 {
        a.pop();
    }
    a
}
fn mag_is_zero(a: &[u8]) -> bool {
    a.iter().all(|&d| d == 0)
}
...
fn mag_divmod(num: &[u8], den: &[u8]) -> Option<(Vec<u8>, Vec<u8>)> {
    if mag_is_zero(den) {
        return None;
    }
...
fn bc_parse(s: &str) -> (bool, Vec<u8>, usize) {
    let s = s.trim();
...
fn bc_format(neg: bool, mag: &[u8], scale: usize, out_scale: usize) -> String {
...
fn bc_div(a: &str, b: &str, scale: usize) -> Option<String> {
    let (na, ma, sa) = bc_parse(a);
    let (nb, mb, sb) = bc_parse(b);
    if mag_is_zero(&mb) {
        return None;
...
fn bc_mod(a: &str, b: &str, scale: usize) -> Option<String> {
...
fn bc_comp(a: &str, b: &str, scale: usize) -> i64 {
...
fn bc_pow(a: &str, exp: &str, scale: usize) -> String {
...
fn bc_sqrt(a: &str, scale: usize) -> Option<String> {
    let (na, ma, sa) = bc_parse(a);
    if na {
        return None;
    }
